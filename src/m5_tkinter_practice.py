...
def main():
    ...
    #      n = int(s)
    ####################################################################
    int_entry_box = ttk.Entry(frame1)
    int_entry_box.grid()

    hello_goodbye_button = ttk.Button(frame1, text='Print N times')
    hello_goodbye_button['command'] = (lambda: print_n_times(my_entry_box, int_entry_box))
    hello_goodbye_button.grid()
...
def print_n_times(string, integer):
    str = string.get()
    number = int(integer.get())
